...
import { listSkills, createSkill, updateSkill, deleteSkill } from '@/modules/cv/services/cv.services';
import type { CreateCVSkillRequest, UpdateCVSkillRequest, CVSkillResponse } from '@/modules/cv/types/cv.types';
...
export function useCreateSkill({ cvId }: { readonly cvId: string }) {
    ...
    return useMutation({
        mutationFn: (skillData: CreateCVSkillRequest) =>
            createSkill({ cvId, skillData }),
        onMutate: async (newSkill) => {
            ...
            if (previousSkills) {
                ...
                queryClient.setQueryData<CVSkillResponse[]>(
                    genSkillsKey(cvId),
                    [...previousSkills, optimisticSkill]
                );
            }
...
export function useUpdateSkill({ cvId }: { readonly cvId: string }) {
    ...
    return useMutation({
        mutationFn: ({
            ...
            skillId: string;
            skillData: UpdateCVSkillRequest;
