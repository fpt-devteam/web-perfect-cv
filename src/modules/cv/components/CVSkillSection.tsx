...
import {
    ...
    useUpdateSkill,
    useDeleteSkill
} from '@/modules/cv/hooks/useSkill';
import { SearchableInput } from '@/shared/components/ui/searchable-input';
import { searchSkillCategories } from '@/modules/cv/services/search.service';
import type { CVSkillResponse, CreateCVSkillRequest, UpdateCVSkillRequest } from '@/modules/cv/types/cv.types';
...
export function CVSkillSection({ cvId }: { readonly cvId: string }) {
    return <CVSkillForm cvId={cvId} />;
}

export function CVSkillForm({ cvId, onSuccess }: CVSkillFormProps) {
    const { showError, showSuccess } = useNotification();
    const { data: skills, isLoading: isLoadingSkills } = useListSkills({ cvId });
    const [isCreating, setIsCreating] = useState(false);
    const [editingSkill, setEditingSkill] = useState<CVSkillResponse | null>(null);
    ...
    const isAnyMutationInProgress = createSkill.isPending || updateSkill.isPending || deleteSkill.isPending;
    ...
    const onSubmit = async (data: SkillFormValues) => {
        ...
        try {
            if (editingSkill) {
                const updateData: UpdateCVSkillRequest = {
                ...
                const createData: CreateCVSkillRequest = {
                    categoryName: data.categoryName,
                    description: data.description,
                };
    ...
    const groupedSkills = skills?.reduce((acc, skill) => {
        ...
        return acc;
    }, {} as Record<string, CVSkillResponse[]>) || {};
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center gap-1 ml-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    ...
                                                    <Button
                                                        ...
                                                        disabled={deletingSkillId === skill.id || isCreating || !!editingSkill || isAnyMutationInProgress}
                                                    >
